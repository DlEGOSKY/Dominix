...
const STRINGS: Record<string, Record<Language, string>> = {
  ...
  "gb.undo": { es: "Deshacer", en: "Undo" },
  "gb.undoTitle": { es: "Deshacer ultima jugada (tecla U)", en: "Undo last play (U key)" },
  "gb.questLabel": { es: "Reto de ronda", en: "Round quest" },
  "gb.questPassed": { es: "Reto superado: {title} \u00b7 +{amount} oro", en: "Quest cleared: {title} \u00b7 +{amount} gold" },
  "gb.questFailed": { es: "Reto fallado: {title}", en: "Quest failed: {title}" },

  // GameBoard active boss restriction chip — labels (concise) and detail (one-liner)
  "gb.r.no_doubles.label": { es: "Sin dobles", en: "No doubles" },
  "gb.r.no_doubles.detail": { es: "Las fichas dobles estan prohibidas", en: "Double tiles are forbidden" },
  "gb.r.only_doubles.label": { es: "Solo dobles", en: "Doubles only" },
  "gb.r.only_doubles.detail": { es: "Unicamente fichas dobles son validas", en: "Only doubles are valid" },
  "gb.r.no_wild.label": { es: "Sin comodines", en: "No wilds" },
  "gb.r.no_wild.detail": { es: "Las fichas comodin no tienen efecto", en: "Wild tiles have no effect" },
  "gb.r.max_tiles.label": { es: "Maximo {n} fichas", en: "Max {n} tiles" },
  "gb.r.max_tiles.detail": { es: "La cadena no puede exceder este limite", en: "Chain cannot exceed this limit" },
  "gb.r.min_patterns.label": { es: "{n} patrones requeridos", en: "{n} patterns required" },
  "gb.r.min_patterns.detail": { es: "Debes activar al menos este numero de patrones", en: "You must trigger at least this many patterns" },
  "gb.r.min_chain_length.label": { es: "Minimo {n} fichas en cadena", en: "Min {n} tiles in chain" },
  "gb.r.min_chain_length.detail": { es: "Necesitas llegar a esta longitud para ganar", en: "You must reach this length to win" },
  "gb.r.only_low.label": { es: "Solo fichas suma <= {n}", en: "Tiles with sum <= {n} only" },
  "gb.r.only_low.detail": { es: "Valores mas altos estan bloqueados", en: "Higher values are blocked" },
  "gb.r.no_repeat_number.label": { es: "Sin numero consecutivo repetido", en: "No consecutive repeated number" },
  "gb.r.no_repeat_number.detail": { es: "El numero de conexion debe cambiar con cada ficha", en: "The connection number must change every tile" },
  "gb.r.max_doubles.label": { es: "Maximo {n} dobles", en: "Max {n} doubles" },
  "gb.r.max_doubles.detail": { es: "Solo puedes colocar esta cantidad de dobles en la cadena", en: "Only this many doubles allowed in the chain" },
  "gb.r.even_sum_only.label": { es: "Solo sumas pares", en: "Even sums only" },
  "gb.r.even_sum_only.detail": { es: "Las fichas con suma impar estan bloqueadas", en: "Odd-sum tiles are blocked" },
  "gb.r.exact_chain_length.label": { es: "Exactamente {n} fichas", en: "Exactly {n} tiles" },
  "gb.r.exact_chain_length.detail": { es: "La cadena debe tener precisamente esta longitud al terminar", en: "Chain must end at exactly this length" },

  // GameBoard floating toasts
  "gb.toast.actionsGain": { es: "+{n} acciones", en: "+{n} actions" },
  "gb.toast.goldGain": { es: "+{n} oro", en: "+{n} gold" },
  "gb.toast.handRestored": { es: "Mano restaurada", en: "Hand restored" },
  "gb.toast.x2Next": { es: "x2 proxima ficha", en: "x2 next tile" },
  "gb.toast.foilApplied": { es: "Foil aplicado", en: "Foil applied" },
  "gb.toast.purge": { es: "Purga", en: "Purge" },
  "gb.toast.forged": { es: "Forjada: {edition}", en: "Forged: {edition}" },
  "gb.toast.pact": { es: "Pacto +{n}{suffix}", en: "Pact +{n}{suffix}" },
  "gb.toast.bossRage": { es: "Jefe se enfurece", en: "Boss enrages" },
  "gb.toast.cosmicAlignment": { es: "Alineacion Cosmica", en: "Cosmic Alignment" },
  "gb.toast.alignment": { es: "Alineacion {name}", en: "{name} Alignment" },
  "gb.toast.setActivated": { es: "Set {name} activado", en: "{name} set activated" },
  "gb.toast.legacy": { es: "Legado: {parts}", en: "Legacy: {parts}" },
  "gb.toast.chaos": { es: "Caos: {name}", en: "Chaos: {name}" },
  "gb.toast.newRelic": { es: "Nueva reliquia: {name}", en: "New relic: {name}" },
